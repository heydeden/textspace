...
export const GET = async (req: Request) => {
  ...
  const rows = await query(
    `SELECT id, username, display_name, bio, role, points, banned, verified, created_at FROM profiles WHERE username = $1`,
    [username]
  );
...
export const PATCH = withUser(async (req, user) => {
  ...
  params.push(user.id);
  const rows = await query(
    `UPDATE profiles SET ${updates.join(', ')} WHERE id = $${idx} RETURNING id, username, display_name, bio, role, points, verified`,
    params
  );
  return ok(rows[0]);
});
