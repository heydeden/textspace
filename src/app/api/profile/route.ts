...
export const GET = async (req: Request) => {
  ...
  const rows = await query(
    `SELECT id, username, display_name, bio, role, banned, verified, theme, avatar_style, avatar_seed, created_at,
...
export const PATCH = withUser(async (req, user) => {
  ...
  params.push(user.id);
  const rows = await query(
    `UPDATE profiles SET ${updates.join(', ')} WHERE id = $${idx} RETURNING id, username, display_name, bio, role, verified, name_effect, theme, avatar_style, avatar_seed`,
    params
  );
  return ok(rows[0]);
});
